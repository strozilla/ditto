...
const EditPost = () => {
    ...
    switch (action.type) {
      ...
      case "bodyRules":
        if (!action.value.trim()) {
          ...
          draft.body.message = "Write a post..."
        }
        return
      case "notFound":
        draft.notFound = true
      ...
      try {
        ...
        if (response.data) {
          ...
          if (appState.user.username != response.data.author.username) {
            appDispatch({ type: "flashMessage", value: "You don't have permission to edit this." })
  ...
  if (state.isFetching) return <Page title="..."><div>Loading...</div></Page>
  ...
  return (
    ...
    <form className='mt-3' onSubmit={submitHandler}>
        ...
        <button className="btn btn-primary" disabled={state.isSaving}>Save Edit</button>
    </form>
    </Page>
  )
}
